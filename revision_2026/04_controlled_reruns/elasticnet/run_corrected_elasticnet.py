...
DEFINITION_PATH = OUT_DIR / "elasticnet_definition_and_provenance.md"
...
def main() -> None:
    ...
    if not DEFINITION_PATH.exists() or not SELECTION_PLAN_PATH.exists():
        raise RuntimeError(
            ...
            "elasticnet_definition_and_provenance.md and/or elasticnet_selection_plan.json"
        )
    ...
    run_id = f"elasticnet_corrected_{git_commit[:12]}_{dataset_sha[:8]}_{split_sha[:8]}"
    ...
    selected_cfg_by_fold: Dict[int, Dict[int, Dict[str, float]]] = {h: {} for h in HORIZONS}
    ...
    for h in HORIZONS:
        ...
        for fold in OUTER_FOLDS:
                ...
                candidate_metrics.append(
                    {
                        "alpha": alpha,
                        "l1_ratio": l1_ratio,
                        "candidate_name": f"enet_a{alpha}_l{l1_ratio}",
            ...
            selected_cfg = sorted(tied, key=lambda c: (c["alpha"], c["l1_ratio"]))[0]
            selected_alpha = float(selected_cfg["alpha"])
            selected_l1_ratio = float(selected_cfg["l1_ratio"])
            selected_cfg_by_fold[h][fold] = {
            ...
            for c in candidate_metrics:
                is_selected = (
                    abs(c["alpha"] - selected_alpha) <= TIE_TOL
                    and abs(c["l1_ratio"] - selected_l1_ratio) <= TIE_TOL
                )
                selection_reason = (
                    "selected_min_inner_validation_MAE_with_tie_break_smallest_alpha_then_l1_ratio"
                    if is_selected
            ...
            selected_rows.append(
                {
                    ...
                    "selected_candidate_name": f"enet_a{selected_alpha}_l{selected_l1_ratio}",
    ...
    if bool(pred.duplicated(subset=["horizon", "outer_fold", "feature_date"]).any()):
        raise RuntimeError("Duplicate elasticnet prediction keys detected")
    ...
    if expected_keys != got_keys:
        ...
        raise RuntimeError(
            f"ElasticNet test-date set mismatch; missing={missing}; extra={extra}"
        )

    pred.to_csv(PREDICTIONS_PATH, index=False)

    selection_df = pd.DataFrame(selection_rows).sort_values(
        ["horizon", "outer_fold", "candidate_alpha", "candidate_l1_ratio"]
    )
    ...
    for h in HORIZONS:
        for fold in OUTER_FOLDS:
            ...
            if d.empty:
                raise RuntimeError(f"Missing elasticnet predictions for H{h} fold{fold}")
    ...
    for h in HORIZONS:
        ...
        if d.empty:
            raise RuntimeError(f"Missing pooled elasticnet predictions for H{h}")
    ...
    manifest = {
        ...
        "selected_configs_by_fold": {
            str(h): {
                str(f): {
                    "alpha": float(selected_cfg_by_fold[h][f]["alpha"]),
                    "l1_ratio": float(selected_cfg_by_fold[h][f]["l1_ratio"]),
                }
        ...
        "notes": "ElasticNet rebuilt under corrected purged nested protocol with fold-local inner (alpha,l1_ratio) selection and strict outer-test isolation.",
    }
