...
RUN_SCRIPT = OUT_DIR / "run_corrected_ridge.py"
...
def update_manifest_test_result(success: bool, tests_run: int, failures: int, errors: int) -> None:
    ...
    payload["test_result"] = (
        "pass_15_of_15" if success else f"fail_{failures + errors}_of_15"
    )
    payload["test_command"] = f"{sys.executable} {Path(__file__).resolve()}"
    payload["test_summary"] = (
        f"Ran {tests_run} tests; failures={failures}; errors={errors}."
    )
    MANIFEST_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")


class TestCorrectedRidge(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.inner["inner_train_date_start"] = normalize_date_col(cls.inner["inner_train_date_start"])
        cls.inner["inner_train_date_end"] = normalize_date_col(cls.inner["inner_train_date_end"])
        cls.inner["inner_validation_date_start"] = normalize_date_col(cls.inner["inner_validation_date_start"])
        cls.inner["inner_validation_date_end"] = normalize_date_col(cls.inner["inner_validation_date_end"])
    ...
    def _mase_denom(self, h: int, fold: int) -> float:
        tr = self.split[
            ...
            & (~self.split["purged_outer_boundary"].astype(str).str.lower().eq("true") if self.split["purged_outer_boundary"].dtype == object else ~self.split["purged_outer_boundary"].astype(bool))
    ...
    def test_08_outer_test_outcomes_not_accessed_during_selection(self) -> None:
        ...
        for h in HORIZONS:
            for fold in FOLDS:
                ...
                self.assertEqual(len(g), len(self.selection_plan["candidate_alphas"]))

                best_mae = float(g["inner_validation_MAE"].min())
                tied = g[np.abs(g["inner_validation_MAE"] - best_mae) <= 1e-12]
                expected_alpha = float(tied["candidate_alpha"].min())

                selected = g[g["selected"] == True]
                self.assertEqual(len(selected), 1)
                self.assertAlmostEqual(float(selected.iloc[0]["candidate_alpha"]), expected_alpha, places=12)

    def test_09_selected_alpha_belongs_to_candidate_grid(self) -> None:
        grid = set(float(x) for x in self.selection_plan["candidate_alphas"])
        sel = set(float(x) for x in self.selected_cfg["selected_alpha"].tolist())
    ...
    def test_13_ridge_and_persistence_event_counts_match_on_identical_dates(self) -> None:
        ridge_dates = self.pred[["horizon", "outer_fold", "feature_date"]].drop_duplicates()
        pers_dates = self.persistence_pred[["horizon", "outer_fold", "feature_date"]].drop_duplicates()
        self.assertEqual(
            set(tuple(x) for x in ridge_dates.itertuples(index=False, name=None)),
            set(tuple(x) for x in pers_dates.itertuples(index=False, name=None)),
        )

        rp = self.prevalence[["horizon", "outer_fold", "events_tau15", "events_tau16", "events_tau17"]].copy()
        pp = self.persistence_prev[["horizon", "outer_fold", "events_tau15", "events_tau16", "events_tau17"]].copy()
        merged = rp.merge(pp, on=["horizon", "outer_fold"], suffixes=("_ridge", "_pers"))
        self.assertEqual(len(merged), 9)

        for tau in [15, 16, 17]:
            self.assertTrue(bool((merged[f"events_tau{tau}_ridge"] == merged[f"events_tau{tau}_pers"]).all()))
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(TestCorrectedRidge)
